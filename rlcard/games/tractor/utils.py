...
def get_valid_cards(first_player, playable_cards):
    ...
    if len(matched_playable_cards) > 0:
        ...
        for suit in ['S', 'H', 'C', 'D', 'J']:
            valid_cards.append(['pass_{}'.format(suit)])

        # - 'pass_score'
        for suit in ['S', 'H', 'C', 'D', 'J']:
            valid_cards.append(['scor_{}'.format(suit)])

    return valid_cards

def sort_units(unit_1, unit_2, is_get_score, pair_first, suit_sequence, trump):
    # compare suit
    suit_1 = get_suit(unit_1[0])
    suit_2 = get_suit(unit_2[0])
    if (suit_1 != suit_2):
        if suit_sequence.index(suit_1) < suit_sequence.index(suit_2):
            return -1
        else:
            return 1

    # if pair_first, compare single vs pair before checking score
    if pair_first and suit_1 == suit_sequence[0]:
        if len(unit_1) > len(unit_2):
            return -1
        elif len(unit_1) < len(unit_2):
            return 1

    # compare score
    score_1 = calc_score([unit_1[0]], trump)
    score_2 = calc_score([unit_2[0]], trump)
    if (score_1 > 0 and score_2 == 0):
        return -1 if is_get_score else 1
    elif (score_1 == 0 and score_2 > 0):
        return 1 if is_get_score else -1

    # compare single vs pair
    if len(unit_1) > len(unit_2):
        return 1
    elif len(unit_1) < len(unit_2):
        return -1

    # if both have scores, and same pattern(single vs pair), then rank the one with higher score first
    if score_1 > 0 and score_2 > 0:
        if score_1 > score_2:
            return -1 if is_get_score else 1
    ...
    # return -1 if CARD_RANK_STR.index(unit_1[0]) < CARD_RANK_STR.index(unit_2[0]) else 1
    return -1 if CARD_RANK_DICT[unit_1[0]] < CARD_RANK_DICT[unit_2[0]] else 1

''' 
    Input:
        current_hand: list of card string
        playable_cards: list of list of card string
        suit_sequence: list of int, always length 5
'''
def get_pass_cards_sequence(hand, playable_cards, is_get_score, pair_first, suit_sequence, trump):
    units = []
    ...
    for cards in playable_cards:
        if len(cards) == 2:  # a pair
            ...
            for cardstr in cards:
                for _, remain_card in enumerate(current_hand):
                    if cardstr == remain_card:
                        current_hand.remove(current_hand[_])
                        break
    for card in current_hand:
        units.append([card])

    units.sort(key=functools.cmp_to_key(
        functools.partial(sort_units, 
                          is_get_score=is_get_score, 
                          pair_first=pair_first,
                          suit_sequence=suit_sequence,
                          trump=trump)
        ))

    sorted_card_list = [item for sublist in units for item in sublist]
