...
def hand2type(cards, trump):
    ...
    ret_type = (len(cards) // 3 + 1) * 10
    
    # check suit
    if (cards[0] == trump[0] or cards[1] == trump[1] or cards[1] == 'J'):
        ret_type += 4
    else:
        ret_type += SUIT_RANK[cards[1]]
...
def tractor_sort_card(card_1, card_2):
    ...
    key = []
    for card in [card_1, card_2]:
        key.append(CARD_RANK_STR.index(str(card)))
    if key[0] > key[1]:
        return 1
    if key[0] < key[1]:
        return -1
...
def get_valid_cards(first_player, playable_cards, trump):
    ...
    '''
    valid_cards = []
    target_hand = first_player.played_cards # string
...
def encode_cards(plane, cards):
    ...
    Args:
        cards (string): string of list of cards
    '''
    if not cards:
        return None
